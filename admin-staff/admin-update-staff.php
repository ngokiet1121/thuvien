...
$query		= "SELECT * FROM tbl_user WHERE id='$staff_id'";
...
if (isset($_POST ["dang_ky"])){
	...
	if($email && $password && $phone)
	{
		...
		//Them moi nguoi dung
		$time	=  date('U');
		$query	= "UPDATE tbl_staff SET f_name='$f_name', l_name='$l_name', email='$email', b_day='$b_day', b_month='$b_month', 
		b_year='$b_year', gender='$gender', address='$address', phone='$phone', type='$type' WHERE id='$staff_id'";
		echo $query;
	...
	else
	{
		...
		$query 	= "SELECT *FROM tbl_staff WHERE email ='$email'";
		...
		if ($num == 0)
		{
			...
			b_year='$b_year',password='$password',gender='$gender',address='$address',phone='$phone',status='$status' WHERE id='$staff_id'";
...
<div id="content_ad">
		...
		<div class="right_ad">
			<div class="adnw-pblsh span">
				<span>CHỈNH SỬA THÔNG TIN STAFF</span>
			</div>
			...
			<form action="<?php $_SERVER["PHP_SELF"]; ?>" method="post">
				<table>
					...
					<tr>
						<td><input class="long" type="password" name="password" placeholder="Email" value="<?php echo $password; ?>"/></td>
					</tr>
					<tr>
